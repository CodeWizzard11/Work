...
import { DentalCheckUpPage } from './components/DentalCheckUpPage';
import { DentalHygienePage } from './components/DentalHygienePage';
import { DentalImplantsPage } from './components/DentalImplantsPage';
import { CustomNightGuardsPage } from './components/CustomNightGuardsPage';
import { SedationDentistryPage } from './components/SedationDentistryPage';
import { TeethWhiteningPage } from './components/TeethWhiteningPage';
import { FeesFinancingPage } from './components/FeesFinancingPage';
...
export default function App() {
  ...
  const handleSelectTab = (tab: ActiveTab) => {
    ...
    if (tab === 'check-up' || tab === 'dental-check-up' || tab === 'dental-full-check-up' || tab === 'dental-exam' || tab === 'exam') {
      setActiveTab('check-up');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'hygiene' || tab === 'cleaning' || tab === 'dental-hygiene' || tab === 'dental-cleaning' || tab === 'hygiene-cleaning') {
      setActiveTab('hygiene');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'implants' || tab === 'dental-implants') {
      setActiveTab('implants');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'custom-night-guards' || tab === 'night-guards') {
      setActiveTab('custom-night-guards');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'sedation' || tab === 'sedation-dentistry') {
      setActiveTab('sedation-dentistry');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'teeth-whitening' || tab === 'whitening') {
      setActiveTab('teeth-whitening');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (tab === 'fees-financing' || tab === 'financing') {
      setActiveTab('fees-financing');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
      ...
      <main className="flex-1 w-full">
        {activeTab === 'tmj' ? (
          ...
          <FillingsPage 
            ...
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'check-up' || activeTab === 'dental-check-up' || activeTab === 'dental-full-check-up' || activeTab === 'dental-exam' || activeTab === 'exam' ? (
          <DentalCheckUpPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Comprehensive Dental Exam / Check Up', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'hygiene' || activeTab === 'cleaning' || activeTab === 'dental-hygiene' || activeTab === 'dental-cleaning' || activeTab === 'hygiene-cleaning' ? (
          <DentalHygienePage 
            onOpenBooking={(note) => handleOpenBookingWithService('Professional Dental Hygiene / Cleaning', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'implants' || activeTab === 'dental-implants' ? (
          <DentalImplantsPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Dental Implants Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'custom-night-guards' || activeTab === 'night-guards' ? (
          <CustomNightGuardsPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Custom Night Guards Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'sedation' || activeTab === 'sedation-dentistry' ? (
          <SedationDentistryPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Sedation Dentistry Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'teeth-whitening' || activeTab === 'whitening' ? (
          <TeethWhiteningPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Teeth Whitening Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'fees-financing' || activeTab === 'financing' ? (
          <FeesFinancingPage 
            onOpenBooking={(note) => handleOpenBookingWithService('Fees & Financing Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          />
        ) : activeTab === 'contact' ? (
          <ContactUsPage 
            onOpenBooking={(service, note) => handleOpenBookingWithService(service || 'General Consultation', note)}
            onNavigateHome={() => handleSelectTab('home')}
          ...
          <>
            ...
            <InfoCards 
              ...
              onSelectInsurance={() => handleSelectTab('fees-financing')}
            />
